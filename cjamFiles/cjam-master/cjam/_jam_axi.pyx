...
# case for user selected multiple dark components    
def axisymmetric(xp, yp, tracer_mge, potential_mge, distance, nonLuminousMass, nonLuminousRadius, beta=0, kappa=0, nscale=1, mscale=1, incl=np.pi/2*u.rad, mbh=0*u.Msun, rbh=0*u.arcsec, nrad=30, nang=7):

    # case for one dm component
#def axisymmetric(xp, yp, tracer_mge, potential_mge, distance, beta=0, kappa=0, nscale=1, mscale=1, incl=np.pi/2*u.rad, mbh=0*u.Msun, rbh=0*u.arcsec, nrad=30, nang=7):
    ...
    # add dark components to potential gaussian
    
    for i in range(len(nonLuminousMass)):
        if nonLuminousMass[i] > 0 and nonLuminousRadius[i] > 0:
            potential_copy.add_row()
            potential_copy["i"][-1] = nonLuminousMass[i]/2/np.pi/(nonLuminousRadius[i]*distance/u.rad).to("pc")**2
            potential_copy["s"][-1] = nonLuminousRadius[i]
